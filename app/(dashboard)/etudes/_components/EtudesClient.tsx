...
import { getEtudes, createEtude, updateEtude, deleteEtude, toggleEtudePublished } from "@/lib/actions/etudes"
...
const STATUT_CONFIG: Record<string, { label: string; chipClass: string; dotClass: string }> = {
  prospection: { label: "Prospection", chipClass: "bg-amber-100 text-amber-700", dotClass: "bg-amber-500" },
  en_cours_prospection: { label: "En cours de prospection", chipClass: "bg-cyan-100 text-cyan-700", dotClass: "bg-cyan-500" },
  signee: { label: "Signée", chipClass: "bg-emerald-100 text-emerald-700", dotClass: "bg-emerald-500" },
  en_cours: { label: "En cours", chipClass: "bg-blue-100 text-blue-700", dotClass: "bg-blue-500" },
  terminee: { label: "Terminée", chipClass: "bg-zinc-100 text-zinc-600", dotClass: "bg-zinc-400" },
}

const STATUT_ORDER: Record<string, number> = {
  prospection: 0, en_cours_prospection: 1, signee: 2, en_cours: 3, terminee: 4,
}

const EMPTY_FORM = { nom: "", numero: "", statut: "prospection", budget: "", budget_ht: "", frais_dossier: "", marge_pct: "", type: "", commentaire: "", client_id: "", suiveur_id: "" }
...
export function EtudesClient({
  ...
  initialEtudes: EtudeWithRelations[]
  initialClients: Client[]
  initialMembres: { id: string; prenom: string | null; nom: string | null }[]
  initialTva: number
}) {
  const [etudes, setEtudes] = useState(initialEtudes)
  ...
  const groupedByStatut = etudes.reduce((acc, e) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <p className="text-xs font-medium text-zinc-400 uppercase tracking-widest mb-1">Study Management</p>
        ...
        <button
          onClick={() => { setEditingId(null); setForm(EMPTY_FORM); setShowModal(true); setFormError(null) }}
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${!selectedStatut ? "bg-[#00236f] text-white border-[#00236f]" : "bg-white text-zinc-600 border-zinc-200 hover:bg-zinc-50"}`}
        >
        ...
        {Object.entries(STATUT_CONFIG).map(([statut, config]) => (
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${selectedStatut === statut ? "bg-[#00236f] text-white border-[#00236f]" : "bg-white text-zinc-600 border-zinc-200 hover:bg-zinc-50"}`}
          >
              ...
              <div className="flex items-start justify-between gap-4 mb-5">
                <div className="flex-1">
                  <div className="flex flex-wrap gap-2 mb-3">
                    <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-red-100 text-red-600 uppercase tracking-wide">Priorité haute</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 bg-zinc-50 rounded-xl mb-5">
                ...
                {activeStudy.suiveur && (
                  <div>
                    ...
                    <p className="text-sm font-semibold text-zinc-800">{activeStudy.suiveur.prenom} {activeStudy.suiveur.nom}</p>
                  </div>
                )}
                <div>
                        ...
                        <div className="flex flex-col items-end gap-1 shrink-0">
                          <div className="flex items-center gap-1.5">
                            {!(etude as any).published && (
                              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-700 border border-amber-200">BROUILLON</span>
                            )}
                      ...
                      <button
                        onClick={(e) => {
                          ...
                          setForm({
                            nom: etude.nom ?? "",
                            numero: etude.numero ?? "",
                            statut: etude.statut ?? "prospection",
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            ...
            <form
              onSubmit={async (e) => {
                ...
                setFormError(null)
                const payload = {
                  nom: form.nom, numero: form.numero, statut: form.statut,
                ...
                const result = editingId ? await updateEtude(editingId, payload as any) : await createEtude(payload)
                setSubmitting(false)
            ...
            >
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  ...
                  <input required value={form.nom} onChange={e => setForm(f => ({ ...f, nom: e.target.value }))} placeholder="Ex : Étude marketing Q3" className="w-full px-3 py-2 text-sm border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00236f]/20" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-zinc-600 mb-1">Numéro *</label>
                  <input required value={form.numero} onChange={e => setForm(f => ({ ...f, numero: e.target.value }))} placeholder="Ex : 2024-001" className="w-full px-3 py-2 text-sm border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00236f]/20" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-zinc-600 mb-1">Budget HT (€)</label>
                  <input type="number" value={form.budget_ht} onChange={e => setForm(f => ({ ...f, budget_ht: e.target.value }))} placeholder="Ex : 5000" className="w-full px-3 py-2 text-sm border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00236f]/20" />
                  <p className="text-xs text-zinc-400 mt-1">Budget TTC : {form.budget_ht ? (Number(form.budget_ht) * (1 + tvaRate / 100)).toFixed(2) : "—"} €</p>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-zinc-600 mb-1">Frais de dossier (€)</label>
                  <input type="number" value={form.frais_dossier} onChange={e => setForm(f => ({ ...f, frais_dossier: e.target.value }))} placeholder="Ex : 150" className="w-full px-3 py-2 text-sm border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00236f]/20" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-zinc-600 mb-1">Marge (%)</label>
                  <input type="number" step="0.1" value={form.marge_pct} onChange={e => setForm(f => ({ ...f, marge_pct: e.target.value }))} placeholder="Ex : 10" className="w-full px-3 py-2 text-sm border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00236f]/20" />
                </div>
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="flex flex-wrap gap-x-4 gap-y-1">
                        ...
                        <span className="ml-auto">= Tarif HT : <strong className="text-sm">{tarifTotal.toLocaleString("fr-FR", { maximumFractionDigits: 2 })} €</strong></span>
                      </div>
                    )
                  })()}
                </div>
                <div className="col-span-2">
                  ...
                  <select required value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))} className="w-full px-3 py-2 text-sm border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00236f]/20">
                ...
                <div>
                  ...
                  <select value={form.client_id} onChange={e => setForm(f => ({ ...f, client_id: e.target.value }))} className="w-full px-3 py-2 text-sm border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00236f]/20">
                    <option value="">— Sélectionner —</option>
                    {initialClients.map(c => <option key={c.id} value={c.id}>{c.nom}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-zinc-600 mb-1">Suiveur</label>
                  <select value={form.suiveur_id} onChange={e => setForm(f => ({ ...f, suiveur_id: e.target.value }))} className="w-full px-3 py-2 text-sm border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00236f]/20">
                    <option value="">— Sélectionner —</option>
                    {initialMembres.map(m => <option key={m.id} value={m.id}>{m.prenom} {m.nom}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-zinc-600 mb-1">Statut</label>
                  <select value={form.statut} onChange={e => setForm(f => ({ ...f, statut: e.target.value }))} className="w-full px-3 py-2 text-sm border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00236f]/20">
                    <option value="prospection">Prospection</option>
                ...
                <div className="col-span-2">
                  ...
                  <textarea value={form.commentaire} onChange={e => setForm(f => ({ ...f, commentaire: e.target.value }))} rows={3} placeholder="Notes, contexte..." className="w-full px-3 py-2 text-sm border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00236f]/20 resize-none" />
                </div>
              </div>
              {formError && <p className="text-xs text-red-500 font-medium">{formError}</p>}
              <div className="flex justify-end gap-3 pt-2">
                <button type="button" onClick={() => { setShowModal(false); setEditingId(null) }} className="px-4 py-2 text-sm text-zinc-600 hover:bg-zinc-100 rounded-lg transition-colors">Annuler</button>
                <button type="submit" disabled={submitting} className="flex items-center gap-2 px-5 py-2 text-sm font-semibold bg-[#00236f] text-white rounded-lg hover:bg-[#1e3a8a] transition-colors disabled:opacity-50">
