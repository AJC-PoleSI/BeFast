...
export function MembresTab() {
  ...
  useEffect(() => {
    ...
    if (roleFilter !== "Tous") {
      ...
      // ne remonte que l'id du poste, pas son slug).
      const selected = allRoles.find(r => r.slug === roleFilter)
      const categorie = selected?.categorie ?? "base"
      if (categorie === "bureau" || categorie === "pole") {
        f = f.filter(m =>
          (m.personne_postes ?? []).some(pp => pp.profils_types?.id === selected?.id)
        )
      } else {
        f = f.filter(m => m.profils_types?.slug === roleFilter)
      }
    }
  ...
  return (
    <div className="h-full flex flex-col space-y-4">
      {pendingCount > 0 && (
        <div className="self-start flex items-center gap-2 px-4 py-2 bg-amber-50 text-amber-700 rounded-xl border border-amber-200 text-sm font-semibold">
                    ...
                    <tr key={m.id} className="hover:bg-zinc-50/70 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-wrap items-center gap-1.5">
                          ...
                          {(m.personne_postes ?? []).map((pp) => {
                            const poste = allRoles.find((r) => r.id === pp.profils_types?.id)
