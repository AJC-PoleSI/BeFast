...
import { Users, PiggyBank, Briefcase, School, FileText, Euro, Clock, TrendingUp } from "lucide-react"
...
export interface StatsData {
  ...
  etudesParType: { ao: number; cs: number; prospection: number }
  caRealise: number
  caPrevisionnel: number
  totalJeh: number
  retributionTotal: number
}
...
/* ─── Client view (data already resolved server-side) ───── */
export function StatistiquesClient({ stats }: { stats: StatsData }) {
  const [visible, setVisible] = useState(false)

  // Données déjà chargées côté serveur : on déclenche juste l'animation
  // d'entrée après le premier paint.
  useEffect(() => {
    const id = requestAnimationFrame(() => setTimeout(() => setVisible(true), 50))
