...
export function MembresClient({ initialMembers, initialRoles }: {
  ...
  return (
    <div className="p-8 h-full flex flex-col">
      <div className="mb-6">
        ...
        <p className="text-zinc-500 text-sm mt-1">Voir et modifier les rôles des utilisateurs inscrits.</p>
      </div>

      <div className="bg-white rounded-xl border border-zinc-200 overflow-hidden shadow-sm flex-1 flex flex-col">

        {/* TOOLBAR */}
        <div className="p-4 border-b border-zinc-100 flex flex-col sm:flex-row justify-between gap-3 bg-zinc-50">
            ...
            {["Tous", ...allRoles.map(r => r.slug)].map((f) => (
              <button
                ...
                className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-colors ${
                  filter === f ? "bg-[#00236f] text-white" : "text-zinc-500 hover:text-zinc-700"
                }`}
              >
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 text-sm border border-zinc-200 rounded-lg w-full sm:w-60 focus:outline-none focus:ring-2 focus:ring-[#00236f]/20"
            />
          </div>
        </div>
          ...
          {loading ? (
            <div className="flex items-center justify-center h-64">
              <div className="text-center">
                ...
                <p className="text-sm text-zinc-400">Chargement des utilisateurs...</p>
              </div>
            </div>
          ) : error ? (
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-semibold text-zinc-800">
                              {m.prenom || ""} {m.nom || ""}
                              {!m.prenom && !m.nom && <span className="text-zinc-400 italic">Sans nom</span>}
                            </div>
