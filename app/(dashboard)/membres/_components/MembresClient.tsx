...
const ROLE_MAP: Record<string, { label: string; color: string }> = {
  ...
  ancien_mandat:   { label: "Ancien Mandat",          color: "bg-zinc-100 text-zinc-600 border-zinc-200" },
    ...
    <div ref={ref} className="relative inline-block">
      ...
      {open && (
        <div className="absolute right-0 mt-1 w-52 bg-white border border-zinc-200 rounded-xl shadow-xl z-20">
          <p className="px-3 pt-3 pb-1 text-[10px] font-bold uppercase tracking-wider text-zinc-400">Changer le rôle</p>
          <div className="p-1.5">
            {roles
...
export function MembresClient({
  ...
  initialMembers: PersonneWithRole[]
  initialRoles: ProfilType[]
}) {
  const { isAdmin } = useUser()
  const [members, setMembers] = useState<PersonneWithRole[]>(initialMembers)
  const [allRoles, setAllRoles] = useState<ProfilType[]>(initialRoles)
  const [filteredMembers, setFilteredMembers] = useState<PersonneWithRole[]>(initialMembers)
  ...
  async function handleValidate(personneId: string) {
    setUpdating(personneId)
    const res = await fetch(`/api/admin/personnes/${personneId}`, {
      method: "PATCH",
  ...
  return (
    <div className="p-8 h-full flex flex-col space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-manrope font-black text-[#00236f]">Membres</h1>
          <p className="text-zinc-500 text-sm mt-1">Voir et gérer les membres de votre Junior-Entreprise.</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-zinc-200 overflow-hidden shadow-sm flex-1 flex flex-col">
        ...
        <div className="p-4 border-b border-zinc-100 flex flex-col lg:flex-row justify-between gap-4 bg-zinc-50">
          <div className="flex flex-wrap gap-4">
            ...
            {/* Filtre statut (admins uniquement) */}
            {isAdmin && (
              <div className="space-y-1.5">
        ...
        <div className="overflow-x-auto flex-1">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 text-zinc-400 gap-2">
              ...
              <button onClick={loadMembers} className="mt-2 px-4 py-2 text-sm bg-zinc-200 text-zinc-700 rounded-lg hover:bg-zinc-300">
                Réessayer
              </button>
            </div>
          ) : (
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-zinc-500 uppercase bg-zinc-50/50 border-b border-zinc-200">
                <tr>
                  ...
                  {isAdmin && <th className="px-6 py-4 font-semibold">Statut Compte</th>}
              ...
              <tbody className="divide-y divide-zinc-100">
                {filteredMembers.length === 0 ? (
                  <tr>
                    <td colSpan={isAdmin ? 5 : 4} className="px-6 py-12 text-center text-zinc-400">
                      Aucun membre trouvé.
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={m.id} className="hover:bg-zinc-50/70 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-semibold text-zinc-800">{m.prenom || ""} {m.nom || ""}</div>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {isAdmin && m.account_status !== "validated" && (
                            <button
