  ...
  const bulletinRows = [
    ...
    ...missionsRes.data
      ...
      .map((m: any) => ({
        ...
        // barème COURANT de la mission (la rémunération, déjà exprimée par
        // intervenant pour l'ensemble de ses JEH — pas un tarif par JEH), comme
        // la reprise d'historique de la migration 067. Si le barème a changé
        // depuis le paiement, ce montant n'est pas celui réellement versé —
        // d'où la colonne « Provenance » qui le signale.
        montant: auCentime(Number(m.remuneration ?? 0)),
