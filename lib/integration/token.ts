...
// ════════════════════════════════════════════════════════════════════════
// Contrat d'intégration Befast ↔ RH Manager (voir docs du repo RH Manager).
// Implémenté à l'identique dans les 3 surfaces (Befast, RH, Onboarding).
// Befast est le MAÎTRE / orchestrateur.
// ════════════════════════════════════════════════════════════════════════
...
export const RH_BASE_URL = (
  process.env.RH_BASE_URL ?? "http://localhost:3000"
).replace(/\/+$/, "")

export const ONBOARDING_BASE_URL = (
  process.env.ONBOARDING_BASE_URL ?? "http://localhost:3002"
).replace(/\/+$/, "")

const MAX_SKEW_MS = 5 * 60 * 1000
const SSO_TTL_MS = 120 * 1000
...
export async function postSigned(url: string, body: unknown): Promise<Response> {
