  ...
  if (scope === "mission") {
    const [{ data: m, error: mErr }, params] = await Promise.all([
      sb
        .from("missions")
        .select(
          "*, intervenant:personnes!missions_intervenant_id_fkey(id, prenom, nom, email), etudes(*, client:clients(id, nom, contact_nom, contact_email, contact_phone), suiveur:personnes!etudes_suiveur_id_fkey(id, prenom, nom, email), echeancier_blocs:public_echeancier_blocs!etude_id(*))"
        )
        .eq("id", entityId)
        .single(),
      paramsPromise,
  ...
  if (scope === "etude") {
    const [{ data: e }, params] = await Promise.all([
      sb
        .from("etudes")
        .select(
          "*, client:clients(id, nom, contact_nom, contact_email, contact_phone), suiveur:personnes!etudes_suiveur_id_fkey(id, prenom, nom, email), echeancier_blocs:public_echeancier_blocs!etude_id(*)"
        )
        .eq("id", entityId)
        .single(),
      paramsPromise,
