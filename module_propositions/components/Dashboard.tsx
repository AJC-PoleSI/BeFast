...
export default function Dashboard() {
    ...
    if (res?.data) {
      setPropales(
        res.data.map((p: any) => ({
          ...
          suiviJehCount: p.suivi_jeh_count,
          suiviJehPrice: p.suivi_jeh_price,
          globalFraisAnnexes: p.global_frais_annexes,
          fraisDossier: p.frais_dossier,
          margeJe: p.marge_je,
          status: p.status,
  ...
  const handleGeneratePpt = async (id: string) => {
    ...
    const payload = {
      ...
      frais_dossier: propale.fraisDossier,
      marge_je: propale.margeJe,
